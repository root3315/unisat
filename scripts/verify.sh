...
One-command end-to-end reproducibility check. Runs the full green
pipeline inside the unisat-ci Docker image — no local gcc, cmake,
or pytest required.

Stages:
    1. docker build -t unisat-ci       (cached after first run)
    2. cmake + cmake --build           (firmware host target)
    3. ctest --output-on-failure
    4. pytest -v                       (ground-station suites)
    5. end-to-end SITL beacon demo

Exit codes:
    0   every stage passed — prints "✓ UniSat green. Ready to submit."
    >0  stage failed; last-stage output shown for triage

Environment:
    Requires Docker Desktop (Linux / Windows / macOS) or Docker
    Engine on POSIX. Honours MSYS_NO_PATHCONV on Git Bash.

Related targets:
    make all        — skip Docker, use local toolchain
    make ci         — equivalent invocation via Makefile
    make demo       — only the SITL beacon phase
